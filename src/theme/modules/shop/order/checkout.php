...
JS
    ...
    if(!\Yii::$app->user->isGuest)
    {
        $userModel = \skeeks\cms\models\User::findOne(\Yii::$app->user->id);
        $user = \yii\helpers\ArrayHelper::toArray($userModel);
    }
    ...
    <? foreach ($widget->shopBuyer->relatedProperties as $property) : ?>
        ...
        <? $field = $form->field($widget->shopBuyer->relatedPropertiesModel, $property->code);
        if(isset($user[$property->code])) {
            $old_value = $widget->shopBuyer->relatedPropertiesModel->getAttribute($property->code);
            $field->textInput(['value' => $old_value?$old_value:$user[$property->code]]);
        }
        echo $field;
        ?>
    <? endforeach; ?>
